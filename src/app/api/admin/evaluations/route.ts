...
export async function GET(req: NextRequest) {
  return runAnalyticsReadApi(req, async ({ db }) => {
    ...
    const snap = await db.collection('player_evaluations').where('playerId', '==', playerId).get()
    const evaluations = snap.docs
      .map((d) => parseEvaluation(d.id, d.data() as Record<string, unknown>))
...
export async function POST(req: NextRequest) {
  return runAnalyticsWriteApi(req, async ({ session, db }) => {
    ...
    const ref = await db.collection('player_evaluations').add({
    ...
    await writeAuditLog(db, {
      ...
      entityId: ref.id,
      summary: `Recorded ${data.type} evaluation for ${String(player.name ?? 'player')}`,
      actor: session.sub,
      actorRole: session.role,
    })

    return Response.json({ id: ref.id, total }, { status: 201 })
  })
}
