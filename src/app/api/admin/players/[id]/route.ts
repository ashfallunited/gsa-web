...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const snap = await getAdminDb().collection('players').doc(id).get()
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { team, ...rest } = body as Record<string, unknown> & { team?: string }
  ...
  await getAdminDb().collection('players').doc(id).update(updates)
