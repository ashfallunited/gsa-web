      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 mb-10">
        {cards.map((c) => (
          <Link
            key={c.label}
            href={c.href}
            className="bg-white border border-gray-100 p-6 hover:border-[#01255f]/30 transition-colors"
          >
            <p className="text-[10px] uppercase tracking-widest font-bold text-[#5a6478] mb-2">{c.label}</p>
            <p className="text-3xl font-bold text-[#01255f]" style={{ fontFamily: 'var(--font-heading)' }}>
              {c.value}
            </p>
